...
var blockView=backbone.View.extend({
    ...
    templateblock:_.template("<tr><td><%= height %></td><td><%= time %></td><td><%= nTx %></td><td><%= size %></td><td><%= weight %></td> <td><a href='./index/bloc/<%= height %>'><button type='button' class='btn btn-warning'>Voir</button></a></td></tr>"   
...
////////////////////////////////////////////////////////////////////////////
    ...
    //Cible to Difficult
    backbone.Events.on('submitDifficulte', function(){ 
        let coeff =  document.getElementById("inputCoefficient").value;
        let exposant = document.getElementById("inputExposant").value;
        ...
        $.ajax({
            ...
            console.log("done difficulte",result)
            $('#result').html(result.difficulte)
        })           
    });
...
//Click dans BoiteOutils    
    ...
    //Difficulte Cible
     $(document).on('click', '#submitDifficulte', function(event){
        console.log("Event")
        Backbone.Events.trigger('submitDifficulte');
    });
